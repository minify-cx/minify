...
int main() {
    ...
    expect(minify::javascript("function outer(){function longHelper(longValue){return longValue;}return longHelper(1);}",structured,err,{minify::OptimizationLevel::Structured}),err);
    eq(structured,"function outer(){function $($){return $}return $(1)}","structured local function declaration binding");
    expect(minify::javascript("const value=function longRecursive(longValue){return longValue?longRecursive(longValue-1):0;};",structured,err,{minify::OptimizationLevel::Structured}),err);
    eq(structured,"const value=function _($){return $?_($-1):0}","structured named function expression ownership");
    expect(minify::javascript("function outer(){exports.value=longHelper\nfunction longHelper(){return 1}return exports.value()}",structured,err,{minify::OptimizationLevel::Structured}),err);
    eq(structured,"function outer(){exports.value=$\nfunction $(){return 1}return exports.value()}","structured ASI function declaration ownership");
